...
from core.event.contracts import EventSpec, HandlerKind, HandlerSpec
from core.event.errors import EventError, EventRegistrationError
...
class RegistrationToken:
    ...
    registration_id: str 
    owner_id: str
    active: bool = True # 注销后标记为无效
    
    # RegistrationToken 内部使用的注销函数, 不对外暴露
    _unregister: Callable[[str], Awaitable[None]] = field(
        repr=False, compare=False, default=lambda _ : _noop()
    )
...
class HandlerRegistration:
    ...
    registration_id: str # Handler 注册 ID
...
class EventRegistry:
    ...
    def __init__(self) -> None:
        ...
        self._tokens: dict[str, RegistrationToken] = {}  # 注册 ID -> 注册凭据
        self._order = 0
    ...
    def register(self, spec: EventSpec) -> RegistrationToken:
        ...
        if spec.event_type in self._events:
            ...
            raise EventRegistrationError(
                EventError(
                    ...
                    message=f"Event type {spec.event_type} is already registered.",
                    details={
        ...
        return self._create_registration_token(registration_id, spec.owner_id)
    ...
    def subscribe(self, spec: HandlerSpec, handler: EventHandler) -> RegistrationToken:
        ...
        if spec.kind == HandlerKind.TRANSFORMER and (
            "*" in spec.event_pattern
            or self.event_spec(spec.event_pattern) is None
        ):
            raise EventRegistrationError(
                EventError(
                    code="registration_conflict",
                    message="Transformer must target one registered exact event.",
                    details={"event_pattern": spec.event_pattern},
                )
            )
        ...
        if key in self._handler_keys:
            raise EventRegistrationError(
                EventError(
                    code="registration_conflict",
                    message=f"Handler is already registered: {spec.handler_id}",
                    details={"owner_id": spec.owner_id},
                )
            )
        registration_id = new_id("handler_reg")
        self._order += 1
        self._handlers[registration_id] = HandlerRegistration(
            registration_id, spec, handler, self._order
        )
    ...
    async def unregister(self, registration_id: str) -> None:
        ...
        # 清理 
        token = self._tokens.pop(registration_id, None)
    ...
    def _create_registration_token(self, registration_id: str, owner_id: str) -> RegistrationToken:
        """创建注册凭据"""
        token = RegistrationToken(
            registration_id=registration_id,
            owner_id=owner_id,
            active=True,
            _unregister=self.unregister
        )
        self._tokens[registration_id] = token
        return token
    ...
    def _validate_event_pattern(pattern: str) -> None:
        ...
        if "*" in pattern and (not pattern.endswith(".*") or pattern.count("*") != 1):
            raise EventRegistrationError(
                EventError(
                    code="registration_conflict",
                    message="Only an exact event type or a trailing wildcard is supported.",
                    details={"event_pattern": pattern},
                )
            )
        if pattern.endswith(".*"):
            prefix = pattern[:-2]
            ...
            if any(
                ...
                raise EventRegistrationError(
                    EventError(
                        code="registration_conflict",
                        message=f"Invalid event pattern: {pattern}",
                        details={"event_pattern": pattern},
                    )
                )
            return
